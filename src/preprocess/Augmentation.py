...
class PermuteChannel(tf.keras.layers.Layer):
    ...
    def __init__(self, p: float = 0.1, **kwargs) -> None:
        """
    ...
    def _permute_channels(self, data: tf.Tensor) -> tf.Tensor:
        """
...
class GaussianNoise(tf.keras.layers.Layer):
    ...
    def __init__(self, p: float = 0.1, stddev: float = .25, **kwargs) -> None:
        """
        Function to initialise the object.
    ...
    def _add_noise(
        ...
        """
        x = tf.cast(data, self.dtype)
        noise = tf.random.normal(tf.shape(x), stddev=self.stddev, dtype=self.dtype)
        return x + noise
    ...
    def get_config(self) -> Mapping[str, float]:
        ...
        Mapping[str, float]
            Dictionary containing the configuration parameters of the object.
        """
        config = {"p": self.p, "stddev": self.stddev}
...
class SpectralMask(tf.keras.layers.Layer):
    ...
    def _apply_single_mask_freq(self, i: int, i_max: int, data: tf.Tensor) -> tf.Tensor:
        """
        ...
        tf.Tensor
            Permuted output data.
        """
        w_mask = tf.random.uniform(shape=(), minval=self.w_min_mask_f, maxval=self.w_max_mask_f + 1, dtype=tf.int32)
        x = data
        x = tf.cond(i < i_max, partial(_Utilities.freq_mask, param=w_mask, data=x),
                    partial(_Utilities.freq_mask, param=0, data=x))
        return i + 1, i_max, x

    def _apply_single_mask_time(self, i: int, i_max: int, data: tf.Tensor) -> tf.Tensor:
        """
        ...
        tf.Tensor
            Permuted output data.
        """
        w_mask = tf.random.uniform(shape=(), minval=self.w_min_mask_t, maxval=self.w_max_mask_t + 1, dtype=tf.int32)
        x = data
        x = tf.cond(i < i_max, partial(_Utilities.time_mask, param=w_mask, data=x),
                    partial(_Utilities.time_mask, param=0, data=x))
        return i + 1, i_max, x

    def _apply_all_mask(self, data: tf.Tensor) -> tf.Tensor:
        """
        ...
        tf.Tensor
            Masked output data.
        """
        n_mask_t = tf.random.uniform(shape=(), maxval=self.n_max_mask_t + 1, dtype=tf.int32)
        n_mask_f = tf.random.uniform(shape=(), maxval=self.n_max_mask_f + 1, dtype=tf.int32)
...
class TimeMask(tf.keras.layers.Layer):
    ...
    def call(self, data: tf.Tensor, training: bool=None) -> tf.Tensor:
        ...
        x = data
        if training:
            w_mask = tf.random.uniform(shape=(), minval=self.w_min_mask, maxval=self.w_max_mask + 1, dtype=tf.int32)
            x = tf.cast(x, self.dtype)
            x = tf.cond(tf.random.uniform(()) < self.p,
...
class FreqMask(tf.keras.layers.Layer):
    ...
    def call(
        ...
        x = data
        if training:
            w_mask = tf.random.uniform((), minval=self.w_min_mask, maxval=self.w_max_mask + 1, dtype=tf.int32)
            x = tf.cast(x, self.dtype)
            x = tf.cond(tf.random.uniform(()) < self.p,
...
class _Utilities(object):
    ...
    def time_mask(
        ...
        condition = tf.math.logical_and(tf.math.greater_equal(indices, t0), tf.math.less(indices, t0 + param))
    ...
    def freq_mask(
        ...
        condition = tf.math.logical_and(tf.math.greater_equal(indices, f0), tf.math.less(indices, f0 + param))
